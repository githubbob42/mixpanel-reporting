...
define(['durandal/system', 'durandal/viewLocator', 'durandal/binder', 'durandal/viewEngine', 'durandal/activator', 'jquery', 'knockout'], function (system, viewLocator, binder, viewEngine, activator, $, ko) {
    ...
    function addDeactivateCallback(context) {        
        if(context.deactivate && context.model && context.model.deactivate) {
            //handle disposal (if KO removes by the template binding)
            ko.utils.domNodeDisposal.addDisposeCallback(context.parent, function() {
                //figure out how to deactivate the module here
                context.model.deactivate();
            });            
        }
    }
    ...
    function tryActivate(context, successCallback, skipActivation) {
        if(skipActivation){
            ...
            if(system.isArray(context.activationData)) {
                result = context.model.activate.apply(context.model, context.activationData || require('plugins/router').activeRouteParams());
            } else {
                result = context.model.activate(context.activationData || require('plugins/router').activeRouteParams());
            }
